...
const SecoundPart = () => {
  return (
    <div className="pt-6 px-2 md:px-20 xl:px-30 2xl:px-48">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mx-auto  gap-4 ">
                ...
                {features.map((item,index)=>(
                    <div key={index} className="border rounded-lg hover:scale-105 duration-500 ease-out p-8  ">
                        <div className="flex items-center justify-center pb-12">
                        <Image  src={item.image} alt={item.alt} width={80} height={80}  />
                        </div>
                        <h2 className="text-2xl bg-gradient-to-r from-black to-gray-100 bg-clip-text text-transparent font-bold pb-4">{item.name}</h2>
                        <p className="bg-gradient-to-r from-gray-800 to-gray-600 text-transparent bg-clip-text text-lg font-bold">{item.description}</p>
                    </div>
                ))}
